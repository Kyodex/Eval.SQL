...
SELECT suppliers.CompanyName, COUNT(Products.SupplierID) as Nbre_produits FROM Suppliers
...
SELECT companyname, COUNT(orders.customerID) AS Nbre_commandes FROM customers
INNER JOIN orders ON customers.CustomerID = orders.customerID
WHERE Country = 'France'
GROUP BY customers.CustomerID
HAVING COUNT(orders.customerID) > 10;
--Requete 5 - Liste des clients ayant un chiffre d’affaires > 30.000 :
SELECT customers.CompanyName, SUM(order_details.UnitPrice*order_details.Quantity) AS CA, customers.Country AS Pays FROM customers
INNER JOIN orders ON customers.CustomerID = orders.CustomerID
...
SELECT SUM(UnitPrice*Quantity) AS Total FROM order_details
INNER JOIN orders ON order_details.OrderID = orders.OrderID
WHERE OrderDate BETWEEN '1997-01-01' AND '1997-12-31';
--Requetes 8 – Montant des ventes de 1997 mois par mois :
SELECT MONTH(orderdate), SUM(UnitPrice*Quantity) AS Total_mois FROM order_details
INNER JOIN orders ON order_details.OrderID = orders.OrderID
WHERE OrderDate BETWEEN '1997-01-01' AND '1997-12-31'
GROUP BY MONTH(orderdate);
--Requetes 9 – Depuis quelle date le client « Du monde entier » n’a plus commandé ?
SELECT MAX(OrderDate) from orders WHERE CustomerID = 'DUMON';
--Requetes 10 – Quel est le délai moyen de livraison en jours ?
SELECT ROUND(AVG(DATEDIFF(ShippedDate, OrderDate))) AS temps_livraison from orders
